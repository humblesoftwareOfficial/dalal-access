...
export const searchContact = (contacts = [], term = "") => {
  try {
    ...
    return contacts.filter((contact) => {
      ...
      return name.includes(searchTerm); //firstName.includes(searchTerm) || lastName.includes(searchTerm) ||
    });
  } catch (error) {
    console.log({ error });
    return null;
...
export const isValidEmail = (email) => {
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return isFieldWithValue(email) && regex.test(email);
};

export const isValidPassword = (value = "") =>
  isFieldWithValue(value) && value.length > 5;

export const EEventAccessType = {
  UNIQUE: "UNIQUE",
  GROUP: "GROUP",
};

export const isValidGuestAccessCode = (code) =>
  code && code.length === 23 && code.includes("GAC-");
...
export const EEventTypeTraduction = {
  WEDDING: "Mariage",
  BAPTISM: "Baptême",
  PARTY: "Soirée",
...
export const getDayOfEvent = (value) => {
  try {
